...
import DropdownModal from "./components/multiSelect";
import PriceModal from "./components/modal-price";
import DescribeModal from "./components/modal-describe";
import ImagesGroup from "./components/imageGroup";
import { useStores } from "../../models";
import { addCommas, checkArrayIsEmptyOrNull, formatCurrency, formatNumber, formatNumberByString, isFormValid, mapDataDistribute, removeNonNumeric, validateFileSize } from "../../utils/validate";
import { G } from "react-native-svg";
import UnitModal from "./components/modal-unit";
...
export const ProductCreateScreen: FC = (item) => {
  ...
  console.log("-----valueSwitchUnit-----------------", valueSwitchUnit)
  const getListUnitGroup = async (valueSwitchUnit: boolean) => {
    ...
    if (unitResult && unitResult.kind === "ok") {
      ...
      let dataModified = data.map((obj: { id: any; name: any }) => {
        // console.log("response__getListUnitGroup", obj.name + "---" + obj.id);
        return {
          id: obj.id,
          label: obj.name,
        };
  ...
  const submitAdd = async () => {
    if (uomId.id === "") {
      ...
      console.log("post-product-data---retailPriceProduct---- : ", JSON.stringify(retailPriceProduct));
      const newArr = dataCreateProduct.map(item => {
        return {
          name: nameProduct + ' - ' + item.name,
          imageUrls: item.imageUrls.map(img => img.result),
    ...
    if (permissionStatus === RESULTS.GRANTED) {
      ...
      launchCamera(options, (response) => {
        ...
        if (response.didCancel) {
          ...
          setImagesNote([...imagesNote, response?.assets[0]]);
          setModalImage(false);
    ...
    try {
      const uploadPromises = imageArray.map(async (image: any, index: any) => {
        ...
        if (checkFileSize) {
          ...
          return await productStore.uploadImages(formData).then(result => ({ result, fileName }));
        }
      });
      ...
      if (results) {
        ...
        if (checkUploadSlider) {
          setImagesURLSlider([...imagesURLSlider, ...results]);
        }
        if (indexItem >= 0) {
          const newArr = dataCreateProduct.slice()
          const newArrImageUri = newArr[indexItem].imageUri.concat(imageArray)
          const newArrImageUrls = newArr[indexItem].imageUrls.concat(results)
          dataCreateProduct[indexItem].imageUri = newArrImageUri
          dataCreateProduct[indexItem].imageUrls = newArrImageUrls
          setDataCreateProduct(newArr)
        } else {
          const updatedData = dataCreateProduct.map(item => {
            return {
              ...item,
              imageUri: [...item.imageUri, ...imageArray],
              imageUrls: [...item.imageUrls, ...results],
            };
          });
  ...
  const handleDeleteImageItem = (index: number) => {
    ...
    const newArrUri = newArr[index].imageUri.slice()
    newArrUrl.splice(productStore.imageModalIndex, 1)
    newArrUri.splice(productStore.imageModalIndex, 1)
    newArr[index].imageUrls = newArrUrl
    newArr[index].imageUri = newArrUri
  ...
  const handleRemoveImage = (index: number, itemImage: any) => {
    console.log("handleRemoveImage Slider---Root", itemImage.name);
    const indexToRemoveLocal = imagesNote.findIndex((item: { name: any; }) => item.name === itemImage.name);
    ...
    const indexToRemove = imagesURLSlider.findIndex((item: { name: any; }) => item.name === itemImage.name);
    if (indexToRemove !== -1) {
      const newImagesURLSlider = [...imagesURLSlider];
      newImagesURLSlider.splice(indexToRemove, 1);
      setImagesURLSlider(newImagesURLSlider);
    }
  };
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      if (imagesNote.length < 6) {
                        ...
                        Toast.show('Số lượng ảnh tối đa là 6', 1,);
                      }
                    }}
              ...
              <Controller
                ...
                render={({ field: { onChange, value, onBlur } }) => (
                  <TextField
                    ...
                    labelDolphin
                    style={{
                      width: scaleWidth(164), flex: 1, justifyContent: 'center', paddingTop: scaleHeight(8)
                    }}
                    inputStyle={{ fontSize: fontSize.size16, fontWeight: "500" }}
                    value={value}
                    onBlur={onBlur}
            ...
            <View
              ...
              <TouchableOpacity
                onPress={() =>
                  ...
                  // navigation.navigate("ChooseSupplierScreen", {selectedIds})
                }
                style={{
                  flexDirection: "row",
                  alignItems: "center",
