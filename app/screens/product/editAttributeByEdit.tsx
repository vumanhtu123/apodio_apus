...
export const EditAttributeByEdit: FC = observer(function EditAttributeByEdit(
  ...
  const { dataAttribute, dropdownSelected, constDataAttribute }: any =
    route?.params;
  ...
  const onPressCaret = (item: any, id: any, idGroup: any) => {
    ...
    const bMap = new Map(
      a.map((item) => [`${item.value}-${item.productAttributeId}`, item])
    );
  ...
  return (
    <View style={styles.ROOT}>
      ...
      <Modal isVisible={showModal} styles={{margin: 0}}>
