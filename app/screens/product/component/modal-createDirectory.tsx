...
const CreateDirectoryModal = (props: any) => {
    ...
    const handleCreateButtonPress = async () => {
        if (name !== '') {
            if (imagesNote) {
                await handleUploadImages(name, imagesNote)
            } else {
    ...
    const handleUploadImages = useCallback(
        async (name: any, imagesNote: any) => {
            const { fileSize, uri, type, fileName } = imagesNote;
            ...
            if (checkFileSize) {
                ...
                const imageUpload = await productStore.uploadImages(formData);
                if (imageUpload) {
                    hideLoading()
                    // setImagesUpload(imageUpload)
                    if (name !== '') {
                        onCreateDirectory(name, imageUpload);
                        // setImagesNote()
                        // setIsVisible(false)
                    }
                } else {
