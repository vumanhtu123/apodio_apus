...
export const ArrangeProduct: FC = () => {
    ...
    const renderProductItem = ({ item }: any) => {
        return (
            <TouchableOpacity onPress={() => toggleTileSelection(item.id)} style={[stylesItem.item, { width: scaleWidth(343), height: scaleHeight(82) }]}>
                <View
                    style={{
                        flexDirection: 'row',
                ...
                >
                    ...
                    <View style={{ alignItems: 'center', justifyContent: 'center', marginLeft: scaleWidth(12) }}>
                        <TouchableOpacity
                            style={[styles.radioButton, selectedTiles.includes(item.id) && styles.radioButtonSelected]}
                            onPress={() => toggleTileSelection(item.id)}
                        >
                            {selectedTiles.includes(item.id) && <Text style={styles.radioButtonText}>{selectedTiles.indexOf(item.id) + 1}</Text>}
                        </TouchableOpacity></View>
                    <View style={{ margin: 6, borderRadius: 10 }}>
                        <AutoImage
                            style={{ width: scaleWidth(70), height: scaleHeight(70), borderRadius: 8 }}
                            source={{
                                uri: "https://vcdn1-dulich.vnecdn.net/2021/07/16/1-1626437591.jpg?w=460&h=0&q=100&dpr=2&fit=crop&s=i2M2IgCcw574LT-bXFY92g",
                            }}
                        />
                    </View>
                    <View style={[stylesItem.titleView, { marginTop: scaleHeight(10), marginHorizontal: scaleWidth(6) }]}>
                        <Text style={stylesItem.title}>{item.name}</Text>
                        {item.description && <Text style={stylesItem.description}>{item.description}</Text>}
        ...
        <View style={styles.ROOT}>
            ...
            <Screen style={styles.ROOT} preset="fixed">
                ...
                <View style={styles.rowNotiType}>
                    <TouchableOpacity onPress={openCategoryModal} style={{ marginRight: scaleWidth(8) }}>
                ...
                <View style={{ alignItems: 'center', marginVertical: 12 }} >
                    <Text style={{ fontWeight: '400', color: '#747475', fontSize: fontSize.size12 }}>Chọn sản phẩm đưa lên đầu</Text>
                </View>
                <View style={{ flex: 0.95, marginHorizontal: 16 }}>
                    <FlatList
                        ...
                        refreshControl={
                            <RefreshControl refreshing={isRefreshing} onRefresh={refreshNotifications} title="ok" />
                        }
                ...
                <View style={{ backgroundColor: 'white', paddingTop: scaleHeight(20) }}>
                    <Button
                        ...
                        style={[
                            styles.btnBottom, { backgroundColor: colors.palette.navyBlue } ,
                            { backgroundColor: (selectedTiles.length === 0) ? "rgb(153,201,238)" : colors.palette.navyBlue}
                        ]}
                        textStyle={[
                            styles.textButton,
