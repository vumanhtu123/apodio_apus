...
const RenderOrderItem = ({ item, index, isGridView, viewProduct, handleProductDetail, handleClassifyDetail, handleAddProduct, handleMinus, handlePlus }: any) => {
    ...
    if (isGridView) {
        return (
            <TouchableOpacity
                key={item.id}
                disabled={item.quantityInventory === 0 || item.quantityInventory < item.minQuantity ? true : false}
                onPress={() => {
                ...
                style={[
                    stylesItem.item,
                    {
                        ...
                        opacity: item.quantityInventory === 0 || item.quantityInventory < item.minQuantity ? 0.5 : 1
                    },
                ]}>
                <View
                    style={{
                ...
                <View
                    ...
                    <View
                        ...
                        {viewProduct === "VIEW_PRODUCT" ? null :
                            (item.isSelect === false ?
                                (item.quantityInventory === 0 || item.quantityInventory < item.minQuantity ? null :
                                ...
                                : <View style={{
                                    ...
                                    <TouchableOpacity onPress={() => handleMinus(item)}
                                        disabled={orderStore.checkPriceList === true ? (item.amount === item.minQuantity || item.amount === Math.ceil(item.minQuantity / item.conversionRate) ? true : false) : (item.amount === 1 ? true : false)}
                                    ...
                                    <TouchableOpacity onPress={() => handlePlus(item)}
                                        disabled={orderStore.checkPriceList === true ? (item.amount === item.quantityInventory || item.amount === Math.floor(item.quantityInventory / item.conversionRate) ? true : false) : (item.amount === item.quantityInventory ? true : false)}
                                        style={{ width: '30%', alignItems: 'center' }}
                                    >
        ...
        return (
            <TouchableOpacity
                ...
                style={[
                    stylesItem.item,
                    { width: scaleWidth(343), opacity: item.quantityInventory === 0 || item.quantityInventory < item.minQuantity ? 0.5 : 1 },
                ]}>
                <View
                    style={{
                        position: "absolute",
